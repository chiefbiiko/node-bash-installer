...
var FALLBACK = { os: 'linux', arch: 'x64', version: undefined }
...
function log_info (lambda_id, ...infos) {
  console.log(`[node-bash-installer lambda ${lambda_id} info]`, ...infos)
}

function log_err (lambda_id, err) {
  console.log(`[node-bash-installer lambda ${lambda_id} error]`, err.stack)
}

function http_panic (lambda_id, res, err, status = 500) {
  log_err(lambda_id, err)
...
function parse_query_params (req) {
  ...
  if (!params.version) 
    params.version = params.v || params['node-version'] || params.node_version
